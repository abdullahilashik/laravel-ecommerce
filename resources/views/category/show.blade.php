<x-frontend title="{{ $category->name }} - Nest">
    @push('styles')
        ...
        <div class="container">
            <div class="archive-header">
                <div class="row align-items-center">
                    ...
                    @endif
                    <div class="col-xl-{{ $category->description ? '3' : '9' }} text-end d-none d-xl-block">
                        <ul class="tags-list">
                            @foreach($categories->take(5) as $cat)
                                ...
                                <li class="hover-up {{ $active ? 'active' : '' }} {{ $loop->last ? 'mr-0' : '' }}">
                                    <a href="{{ $active
                                        ...
                                        : route('category.show', $cat->slug) }}"><i class="fi-rs-tag mr-10"></i>{{ $cat->name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    @empty
                        <div class="col-12">
                            <div class="text-center py-5">
                                ...
                                <a href="{{ route('category.show', $category->slug) }}" class="btn btn-sm btn-default mt-2">Clear filters</a>
                            </div>
                        </div>
                    @endforelse
                </div>
                        ...
                        @foreach($categories as $cat)
                            <li>
                                <a href="{{ route('category.show', $cat->slug) }}">
                                    <img src="{{ asset('assets/imgs/theme/icons/category-' . ((($cat->id - 1) % 11) + 1) . '.svg') }}" alt="" />{{ $cat->name }}
                                </a><span class="count">{{ $cat->products_count }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
